    ...
    <script>
        $(document).ready(function() {
            $('#blogForm').on('submit', function(e) {
                ...
                if ($('#project_image')[0].files.length === 0) {
                    $('#project_image').addClass('is-invalid');
                    $('#project_image').after('<div class="invalid-feedback">Image is required.</div>');
                    valid = false;
                }
                else if ($('#project_image')[0].files.length > 0) {
                    const file = $('#project_image')[0].files[0];
                    const fileType = file.type;

                    if (fileType === 'image/gif') {
                        $('#project_image').addClass('is-invalid');
                        $('#project_image').after('<div class="invalid-feedback">GIF images are not allowed.</div>');
                        valid = false;
                    }
                }
                if (!valid) return;
